...
export const ConnectionCard = ({
  ...
  const handleClick = () => {
    router.push(`/messages/${connection.username}`);
  };
  ...
  return (
    ...
    >
      <Avatar className="w-12 h-12 rounded-full overflow-hidden">
        <AvatarImage src={connection.avatar} alt={connection.username} className="object-cover" />
      ...
      <div className="flex flex-col text-sm gap-1 font-medium text-white">
        <span className="cursor-pointer hover:underline">{connection.username}</span>
